...
expName = 'First Attempt at Part 3 Expt Script copy'  # from the Builder filename that created this script
expInfo = {'participant': ''}
...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='C:\\Users\\Chris\\Documents\\Music Interestingness in the Brain\\PsychoPy Paradigm\\First Attempt at Part 3 Expt Script copy.py',
...
top_instr_txt = visual.TextStim(win=win, name='top_instr_txt',
    text='Welcome to Part 3 of the experiment. The instructions are as follows:\n\nAt the start of each trial, you will be instructed either "Attend to the music", or "Do not attend to the music", before a random clip is played. A silent film will also be played. Try to listen to the music if instructed to, but you can enjoy the film as well. Please try not to blink or move during this 30s period.\n\nAs this is a passive test, you will not need to do anything else.\n\nThere will then be a 25s break (blink/stretch/etc as much as you like during this), before the next trial begins.\n\nAfter eight trials, there will be a five minute break, before the final seven trials run.\n\nIf you have any questions at all please ask the experimenters.',
    font='Open Sans',
    pos=(0, 0.1), height=0.04, wrapWidth=1.7, ori=0.0, 
...
top_instr_txt_2 = visual.TextStim(win=win, name='top_instr_txt_2',
    text='Welcome to Part 3 of the experiment. The instructions are as follows:\n\nFirst, you will be told if you should attend, or not attend, to the music piece that will be played.\n\nThen, a random music clip will be played. A silent film will also be played. Remember, nou will not have to answer any questions during the trials - just try to focus/not focus on the music as instructed. Please try not to blink or move during this 30s period.\n\nThere will then be a 25s break (blink/stretch/etc as much as you like during this), before the next trial begins.\n\nAfter eight trials, there will be a five minute break, before the final seven trials run.\n\nIf you have any questions at all please ask the experimenters.',
    font='Open Sans',
...
block_end = data.TrialHandler(nReps=1.0, method='random', 
    extraInfo=expInfo, originPath=-1,
    trialList=data.importConditions('stimuliList.xlsx', selection=randnums[1:7]),
    seed=None, name='block_end')
thisExp.addLoop(block_end)  # add the loop to the experiment
thisBlock_end = block_end.trialList[0]  # so we can initialise stimuli with some values
# abbreviate parameter names if possible (e.g. rgb = thisBlock_end.rgb)
if thisBlock_end != None:
    for paramName in thisBlock_end:
        exec('{} = thisBlock_end[paramName]'.format(paramName))

for thisBlock_end in block_end:
    currentLoop = block_end
    # abbreviate parameter names if possible (e.g. rgb = thisBlock_end.rgb)
    if thisBlock_end != None:
        for paramName in thisBlock_end:
            exec('{} = thisBlock_end[paramName]'.format(paramName))
    ...
    continueRoutine = True
    # update component parameters for each repeat
    stimulus.setSound(stimuli, secs=30.0, hamming=True)
    ...
    for thisComponent in trialComponents:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    block_end.addData('attendNote.started', attendNote.tStartRefresh)
    block_end.addData('attendNote.stopped', attendNote.tStopRefresh)
    block_end.addData('dontAttendNote.started', dontAttendNote.tStartRefresh)
    block_end.addData('dontAttendNote.stopped', dontAttendNote.tStopRefresh)
    stimulus.stop()  # ensure sound has stopped at end of routine
    block_end.addData('stimulus.started', stimulus.tStartRefresh)
    block_end.addData('stimulus.stopped', stimulus.tStopRefresh)
    ...
    for thisComponent in interTrialPauseComponents:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    block_end.addData('Break25s.started', Break25s.tStartRefresh)
    block_end.addData('Break25s.stopped', Break25s.tStopRefresh)
    thisExp.nextEntry()
    
# completed 1.0 repeats of 'block_end'


# set up handler to look after randomisation of conditions etc
block_1end = data.TrialHandler(nReps=1.0, method='random', 
    extraInfo=expInfo, originPath=-1,
    trialList=data.importConditions('stimuliList.xlsx', selection=randnums[8]),
    seed=None, name='block_1end')
thisExp.addLoop(block_1end)  # add the loop to the experiment
thisBlock_1end = block_1end.trialList[0]  # so we can initialise stimuli with some values
# abbreviate parameter names if possible (e.g. rgb = thisBlock_1end.rgb)
if thisBlock_1end != None:
    for paramName in thisBlock_1end:
        exec('{} = thisBlock_1end[paramName]'.format(paramName))

for thisBlock_1end in block_1end:
    currentLoop = block_1end
    # abbreviate parameter names if possible (e.g. rgb = thisBlock_1end.rgb)
    if thisBlock_1end != None:
        for paramName in thisBlock_1end:
            exec('{} = thisBlock_1end[paramName]'.format(paramName))
    ...
    for thisComponent in trialComponents:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    block_1end.addData('attendNote.started', attendNote.tStartRefresh)
    block_1end.addData('attendNote.stopped', attendNote.tStopRefresh)
    block_1end.addData('dontAttendNote.started', dontAttendNote.tStartRefresh)
    block_1end.addData('dontAttendNote.stopped', dontAttendNote.tStopRefresh)
    stimulus.stop()  # ensure sound has stopped at end of routine
    block_1end.addData('stimulus.started', stimulus.tStartRefresh)
    block_1end.addData('stimulus.stopped', stimulus.tStopRefresh)
    # the Routine "trial" was not non-slip safe, so reset the non-slip timer
    routineTimer.reset()
    thisExp.nextEntry()
    
# completed 1.0 repeats of 'block_1end'
...
# set up handler to look after randomisation of conditions etc
block_2 = data.TrialHandler(nReps=1.0, method='random', 
    extraInfo=expInfo, originPath=-1,
...
for thisBlock_2 in block_2:
    ...
    for thisComponent in trialComponents:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    block_2.addData('attendNote.started', attendNote.tStartRefresh)
    block_2.addData('attendNote.stopped', attendNote.tStopRefresh)
    block_2.addData('dontAttendNote.started', dontAttendNote.tStartRefresh)
    block_2.addData('dontAttendNote.stopped', dontAttendNote.tStopRefresh)
    stimulus.stop()  # ensure sound has stopped at end of routine
    block_2.addData('stimulus.started', stimulus.tStartRefresh)
    block_2.addData('stimulus.stopped', stimulus.tStopRefresh)
    # the Routine "trial" was not non-slip safe, so reset the non-slip timer
    routineTimer.reset()
    ...
    for thisComponent in interTrialPauseComponents:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    block_2.addData('Break25s.started', Break25s.tStartRefresh)
    block_2.addData('Break25s.stopped', Break25s.tStopRefresh)
    thisExp.nextEntry()
    
# completed 1.0 repeats of 'block_2'


# set up handler to look after randomisation of conditions etc
block_2end = data.TrialHandler(nReps=1.0, method='random', 
    extraInfo=expInfo, originPath=-1,
    trialList=data.importConditions('stimuliList.xlsx', selection=randnums[15]),
    seed=None, name='block_2end')
thisExp.addLoop(block_2end)  # add the loop to the experiment
thisBlock_2end = block_2end.trialList[0]  # so we can initialise stimuli with some values
# abbreviate parameter names if possible (e.g. rgb = thisBlock_2end.rgb)
if thisBlock_2end != None:
    for paramName in thisBlock_2end:
        exec('{} = thisBlock_2end[paramName]'.format(paramName))

for thisBlock_2end in block_2end:
    currentLoop = block_2end
    # abbreviate parameter names if possible (e.g. rgb = thisBlock_2end.rgb)
    if thisBlock_2end != None:
        for paramName in thisBlock_2end:
            exec('{} = thisBlock_2end[paramName]'.format(paramName))
    ...
    for thisComponent in trialComponents:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    block_2end.addData('attendNote.started', attendNote.tStartRefresh)
    block_2end.addData('attendNote.stopped', attendNote.tStopRefresh)
    block_2end.addData('dontAttendNote.started', dontAttendNote.tStartRefresh)
    block_2end.addData('dontAttendNote.stopped', dontAttendNote.tStopRefresh)
    stimulus.stop()  # ensure sound has stopped at end of routine
    block_2end.addData('stimulus.started', stimulus.tStartRefresh)
    block_2end.addData('stimulus.stopped', stimulus.tStopRefresh)
    # the Routine "trial" was not non-slip safe, so reset the non-slip timer
    routineTimer.reset()
    thisExp.nextEntry()
    
# completed 1.0 repeats of 'block_2end'
